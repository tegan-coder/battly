...
class Settings {
  ...
  async initTheme() {
    ...
    if (color_db) {
      ...
      color.value = "#3e8ed0";
      color.style.backgroundColor = "#3e8ed0";
    ...
    if (colorHover_db) {
      ...
      localStorage.setItem(
        "theme-color-hover",
        tinycolor(color.value).darken(10).toString()
      );
    }
    ...
    if (color_bottom_bar_db) {
      ...
      color_bottom_bar.style.backgroundColor = "#1f1f1f";
    ...
    if (background_loading_screen_color) {
      document.getElementById("theme-color-loading-screen").value =
        background_loading_screen_color;
    } else {
    ...
    document
      .getElementById("theme-opacity-bottom-bar")
      .addEventListener("input", (e) => {
        ...
        let bottom_bar_settings = document.querySelector(
          ".bottom_bar_settings"
        );
    ...
    document
      .getElementById("theme-color-bottom-bar")
      .addEventListener("input", (e) => {
    ...
    document.getElementById("theme-color").addEventListener("input", (e) => {
      ...
      document.querySelector(".save-tabs-btn").style.backgroundColor =
        e.target.value;
      ...
      localStorage.setItem(
        "theme-color-hover",
        tinycolor(e.target.value).darken(10).toString()
      );
    });
    ...
    document.querySelector(".memory-slider").max =
      /* la RAM total del sistema */ Math.trunc(
      (os.totalmem() / 1073741824) * 10
    ) / 10;

    document
      .getElementById("theme-color-loading-screen")
    ...
    document
      .getElementById("background-input-file")
      .addEventListener("change", (e) => {
    ...
    function handleFileSelect(input) {
      ...
      save.addEventListener("click", (e) => {
        ...
        fs.writeFile(
          path.join(`${dataDirectory}\\.battly\\battly\\background.png`),
          buf,
          function (err) {
    ...
    document
      .getElementById("reproducir-sonido-inicio")
      .addEventListener("click", (e) => {
        let sonido_inicio = new Audio(
          "./assets/audios/" + selectSonidoInicio.value + ".mp3"
        );
  ...
  initAccount() {
    ...
    document.querySelector(".accounts").addEventListener("click", async (e) => {
      ...
      let selectedaccount = await this.database.get(
        "1234",
        "accounts-selected"
      );
  ...
  async Java() {
    ...
    javaPathText.textContent = `${dataDirectory}/${process.platform == "darwin"
    ...
    let javaPath =
      configClient?.java_config?.java_path || lang.java_path_didnt_set;
    ...
    document
      .getElementById("open-explorer-java-path")
    ...
    document
      .getElementById("java-path-reset")
      .addEventListener("click", async () => {
